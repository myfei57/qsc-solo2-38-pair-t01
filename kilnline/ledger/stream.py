...
class EventStream:
    ...
    def uncommitted(self) -> list[LedgerRecord]:
        """Records written but not yet published past the watermark."""
    ...
    def commit(self, *, committed_at: float | None = None) -> int:
        """Publish every staged record, advancing the watermark to the tip."""

        return self.commit_through(self.last_sequence, committed_at=committed_at)

    def commit_through(self, sequence: int, *, committed_at: float | None = None) -> int:
        """Publish the committed prefix up to and including ``sequence``."""

        target = int(sequence)
        if target < self._watermark:
            raise WatermarkError(
                "commit cannot move the watermark backwards",
                watermark=self._watermark,
                requested=target,
            )
        if target > self.last_sequence:
            raise WatermarkError(
                "commit target is beyond the last written record",
                watermark=self._watermark,
                requested=target,
                last_sequence=self.last_sequence,
            )
    ...
    def rollback(self, *, removed_at: float | None = None) -> list[LedgerRecord]:
        """Drop every staged record, keeping the committed prefix on disk."""

        dropped = self.uncommitted()
        if dropped:
            self._records = self.committed()
            self._rewrite()
    ...
    def resolve(self, key: str) -> tuple[LedgerRecord | None, list[int]]:
        ...
        for record in self.committed():
            ...
            if record.is_tombstone:
                ...
                if live is not None and target == live.sequence:
